...
class ATSHomePage extends StatefulWidget {
  ...
  @override
  State<ATSHomePage> createState() => _ATSHomePageState();
}

class _ATSHomePageState extends State<ATSHomePage> {
            ...
            children: [
              ...
              Expanded(
                child: PageView(
                  ...
                  children: const [AtsAppliedPage(), ATSReferalPage()],
                ),
              ),
            ],
          ),
...
class TabClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    if (isLeft) {
      ...
      path.quadraticBezierTo(
        size.width - 0.512 * size.width,
        0,
        size.width - 0.52 * size.width,
        0.1 * size.height,
      );
      path.lineTo(size.width - 0.57 * size.width, 0.83 * size.height);
      path.quadraticBezierTo(
        size.width - 0.58 * size.width,
        0.9 * size.height,
        size.width - 0.59 * size.width,
        0.9 * size.height,
      );
