...
// final leadsDataProvider = StreamProvider<List<LeadDetails>>((ref) {
//   return _NewInterviewBayCCState._fetchLeadsData()
//       .map((data) => data.map((item) => LeadDetails.fromJson(item)).toList());
// });

final fetchAllApplicantProvider = FutureProvider<List<Applicant>>(
    (ref) => _NewInterviewBayCCState.fetchAllApplicants());

class NewInterviewBayCC extends ConsumerStatefulWidget {
  const NewInterviewBayCC({super.key});

  @override
  ConsumerState<NewInterviewBayCC> createState() => _NewInterviewBayCCState();
}

class _NewInterviewBayCCState extends ConsumerState<NewInterviewBayCC>
  ...
  Widget build(BuildContext context) {
    final leadData = ref.watch(fetchAllApplicantProvider);
...
// Remove excluded statuses
        ...
        // statusWithData.forEach((status) {
        //   int count;
        //   if (status == "Not Selected/Not shortlist") {
        //     count = leadController.filteredLeads
        //         .where((lead) =>
        //             excludedStatuses.contains(lead.hrStatus.toString()) &&
        //             (lead.applicantName!.toLowerCase().contains(
        //                       _searchController.text.toLowerCase(),
        //                     ) ||
        //                 lead.companyName!.toLowerCase().contains(
        //                       _searchController.text.toLowerCase(),
        //                     ) ||
        //                 lead.process!.toLowerCase().contains(
        //                       _searchController.text.toLowerCase(),
        //                     )))
        //         .length;
        //   } else {
        //     count = leadController.filteredLeads
        //         .where((applicant) =>
        //             applicant.hrStatus.toString() == status &&
        //             !excludedStatuses.contains(applicant.hrStatus.toString()))
        //         .where((element) =>
                  ...
                  body: TabBarView(
                    ...
                    children: finalStatus.map((status) {
                      final spocData = fetchdata
                          .map((lead) => lead.source_name?.toString())
...
//
  Widget statusView(
    ...
    int selectedRoundIndex =
        item.interview_rounds != null && finalinterviewRounds != null
